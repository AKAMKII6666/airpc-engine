/**
	* 资源库域账本（Zustand）。
	* 切片：列表 / 选中 / loading·loadError / refreshStamp；只结果型 write。
	* 禁网络、禁 import bis / ajaxProxy / next/navigation（STRUCT-022）。
	* 灌账在 shellBis；CRUD 编排在 pageBis；本文件不挂 UI。
	*/
import { create } from "zustand";
import {
	createAssetsSessionSlice,
	type AssetsStoreState,
} from "@studio-v2/src/stores/assets/model/assetsStoreModel";
import { createAssetsListActions } from "@studio-v2/src/stores/assets/writes/assetsStoreListWrites";

export type { AssetsStoreState } from "@studio-v2/src/stores/assets/model/assetsStoreModel";
export { pickAssetsSelectedId } from "@studio-v2/src/stores/assets/model/assetsStoreModel";

export const useAssetsStore = create<AssetsStoreState>((set) => ({
	...createAssetsSessionSlice(),
	refreshStamp: 0,
	...createAssetsListActions(set),
}));
