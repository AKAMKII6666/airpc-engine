...
function hasAttitudeCoreFields(
	payload: Partial<MemoryAttitudeListRow>,
): payload is Pick<MemoryAttitudeListRow, "stance" | "summary" | "evidence"> &
	Partial<MemoryAttitudeListRow> {
	return (
...
function parseAttitudePayload(
	...
	try {
		...
		if (!hasAttitudeCoreFields(payload)) return null;
		return {
			...
			at: asOptionalString(payload.at),
		};
	} catch {
		return null;
	}
...
function emptyPage(page: number, pageSize: number): MemoryListPage {
	return { items: [], attitudes: [], total: 0, page, pageSize };
}

function hasMemoryEntriesTable(db: SqliteDb): boolean {
	const table = db
		.prepare(
...
function queryMemoryPageRows(
	db: SqliteDb,
	input: { userId: string; agentId: string; page: number; pageSize: number },
): { total: number; rows: MemoryEntryRow[]; attitudeRows: AttitudeEntryRow[] } {
	const totalRow = db
		.prepare(
	...
	const rows = db
		...
		.all(input.userId, input.agentId, input.pageSize, offset) as MemoryEntryRow[];
	const attitudeRows = db
		...
		.all(input.userId, input.agentId) as AttitudeEntryRow[];
	return {
...
function mapAttitudeRows(
	attitudeRows: AttitudeEntryRow[],
): MemoryAttitudeListRow[] {
	return attitudeRows.flatMap(function (row) {
...
export function listMemoryPage(input: {
	...
	try {
		...
		const queried = queryMemoryPageRows(db, {
			userId: input.userId,
		...
		return {
			items: queried.rows.map(function (r) {
				return {
					id: r.id,
					layer: r.layer,
					kind: r.kind,
			...
			attitudes: mapAttitudeRows(queried.attitudeRows),
			total: queried.total,
			page,
			pageSize,
		};
