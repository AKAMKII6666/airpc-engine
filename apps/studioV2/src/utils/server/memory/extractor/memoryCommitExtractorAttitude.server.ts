...
function asIntegerList(value: unknown): number[] {
	if (!Array.isArray(value)) return [];
	return value.filter(function (index): index is number {
		return Number.isInteger(index);
	});
}

function hasAttitudeCoreFields(
	item: Partial<MemoryAttitudeExtraction>,
): item is Pick<MemoryAttitudeExtraction, "stance" | "summary" | "evidence"> &
	Partial<MemoryAttitudeExtraction> {
	return (
...
function parseAttitudeExtraction(
	...
	const item = raw as Partial<MemoryAttitudeExtraction>;
	if (!hasAttitudeCoreFields(item)) return null;
	return {
		...
		feel: trimStringList(
			asStringList(item.feel),
			MAX_ATTITUDE_FEEL_CHARS,
			MAX_ATTITUDE_FEEL_TAGS,
		),
		keywords: trimStringList(
			asStringList(item.keywords),
			MAX_ATTITUDE_KEYWORD_CHARS,
			MAX_ATTITUDE_KEYWORDS,
		),
		evidenceTurnIndexes: asIntegerList(item.evidenceTurnIndexes),
	};
}
...
function repairAttitudeEvidenceTurnIndexes(
	...
	if (hasAnyTurnEvidence(transcript, attitude.evidenceTurnIndexes)) {
		return attitude.evidenceTurnIndexes.filter(function (index: number) {
			return Boolean(transcript.turns[index]);
		});
	}
