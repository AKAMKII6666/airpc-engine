...
import type { ServerLlmChatMessage } from "@studio-v2/src/utils/server/llm/client/llmClient.server";
...
const DEFAULT_OPENING_POLICY: OpeningLlmContextPolicy = {
...
function appendFilteredSystemBlocks(
	messages: ServerLlmChatMessage[],
	...
	for (const block of blocks) {
		...
		if (isPromptBlockExcluded(block, policy)) continue;
		pushIfText(messages, "system", block);
	}
}
...
function appendRenderedPrompt(
	...
	if (!prompt) return;
	const effectivePolicy = policy ?? DEFAULT_OPENING_POLICY;
	if (effectivePolicy.includeSystemHard) {
		appendFilteredSystemBlocks(messages, prompt.systemHard, effectivePolicy);
	}
	...
	if (effectivePolicy.includeSoftContext) {
		appendFilteredSystemBlocks(messages, prompt.softContext, effectivePolicy);
	}
...
function sessionAllowsHangupTool(session: CallSession): boolean {
	return (session.frozenTools ?? listToolsForCard(session.frozenCard, {
...
export function buildTurnLlmMessages(
	...
	appendRenderedPrompt(
		...
		session.renderedPrompt,
		undefined,
		sessionAllowsHangupTool(session),
	);
