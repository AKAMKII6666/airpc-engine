...
async function finishStreamTurn(input: {
	host: EngineHost;
	session: CallSession;
	messageId: string;
	emitter: DebuggerMessageStreamEmitter;
	...
	const result = await runDebuggerLlmWithToolsStream({
		...
		messages: buildTurnLlmMessages(input.session),
		temperature: 0.7,
