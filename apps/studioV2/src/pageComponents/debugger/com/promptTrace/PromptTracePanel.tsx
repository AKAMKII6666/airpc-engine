...
import {
	...
	promptTraceItemKey,
	providerClass,
} from "./blocks/PromptTraceBlocks";
import { PromptTraceOpening, PromptTraceTools } from "./blocks/PromptTraceSections";
...
function renderPromptProviderRail(trace: DebuggerPromptTraceView) {
	if (trace.providerRows.length === 0) {
		return <div className={styles.debugEmptyLine}>暂无 provider trace。</div>;
	}
	return (
	...
	function PromptTracePanel({
		...
		return (
			<div className={styles.promptTrace}>
				...
				{renderPromptProviderRail(trace)}
