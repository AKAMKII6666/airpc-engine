...
import { UserDetailLoreSection } from "@studio-v2/src/pageComponents/users/com/lore/UserDetailLoreSection";
...
function renderUserDetailLower(input: {
	profile: UserProfileSummary;
	formik: FormikProps<UserDetailFormValues>;
	lorePreview: LorePreviewDto | null;
	loreBusy: boolean;
	...
	return (
		<>
			...
			<UserDetailLoreSection
				lorePreview={input.lorePreview}
				loreBusy={input.loreBusy}
				disabled={input.formik.isSubmitting}
				onBootstrapLore={input.onBootstrapLore}
			/>
			<div className={styles.section}>
				<h3 className={styles.sectionTitle}>时间信息</h3>
				{/* 引用了Typography组件，用于只读展示创建时间 */}
				<Typography variant="body2" className={styles.detailMeta}>
					创建时间：{input.profile.createdAt}
				</Typography>
				{/* 引用了Typography组件，用于只读展示更新时间 */}
				<Typography variant="body2" className={styles.detailMeta}>
					更新时间：{input.profile.updatedAt}
				</Typography>
			</div>
			<div className={styles.advanced}>
				<h3 className={styles.sectionTitle}>高级信息</h3>
				<div className={styles.advancedId}>userId · {input.profile.userId}</div>
			</div>
			<div className={styles.section}>
				...
				<Button
					type="submit"
					variant="contained"
					disabled={input.formik.isSubmitting || input.loreBusy}
				>
					保存到玩家档案
				</Button>
			</div>
