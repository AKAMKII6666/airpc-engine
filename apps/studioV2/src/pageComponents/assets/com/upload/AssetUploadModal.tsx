...
function useAssetUploadDraft(
	...
	return { file, busy, error, handleFileChange, handleUpload, handleClose };
}
...
export const AssetUploadModal: FC<Props> = function AssetUploadModal({
	...
	const draft = useAssetUploadDraft(onUpload, onClose);
	...
	return (
		...
		<AppModal
			...
			actions={
				<>
					{/* 引用了Button组件，用于取消上传 */}
					<Button color="inherit" disabled={draft.busy} onClick={draft.handleClose}>
						取消
					</Button>
					{/* 引用了Button组件，用于提交上传 */}
					<Button
						variant="contained"
						disabled={!draft.file || draft.busy}
						onClick={() => {
							void draft.handleUpload();
						}}
					>
						上传
					</Button>
		...
		>
			...
			<Button variant="outlined" component="label" disabled={draft.busy}>
				选择文件
				<input hidden type="file" onChange={draft.handleFileChange} />
			</Button>
