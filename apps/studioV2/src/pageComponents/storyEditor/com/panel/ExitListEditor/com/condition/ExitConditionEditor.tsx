...
function ExitConditionReadonly({
	// condition 是引擎 ExitCondition 真源，用于只读预览
	condition,
}: {
...
function ExitConditionOpField({
	// condition 是可编辑叶子条件，用于 op 回显
	condition,
	// onCommit 是写回回调，用于切换 op 后重建条件
	onCommit,
}: {
	condition: ExitCondition & { op: ExitConditionV1Op };
...
function ExitConditionLeafExtras({
	// condition 是可编辑叶子条件，用于字段与预览
	condition,
	// requiredBeats 是本卡必做节拍，用于 beatId Select
	requiredBeats,
	// onCommit 是写回回调，用于同步叶子字段
	onCommit,
}: {
	condition: ExitCondition & { op: ExitConditionV1Op };
	...
	const beatOp =
		condition.op === "beat_completed" || condition.op === "beat_missing";
	return (
		<>
...
export const ExitConditionEditor: FC<ExitConditionEditorProps> =
	function ExitConditionEditor({
		...
		if (!editable) {
			return (
				// 引用了ExitConditionReadonly组件，用于复合条件只读预览
				<ExitConditionReadonly condition={condition} />
			);
		}
		...
		return (
			<div className={styles.conditionBlock}>
				{/* 引用了ExitConditionOpField组件，用于条件 op 选择 */}
				<ExitConditionOpField condition={leaf} onCommit={commit} />
				{/* 引用了ExitConditionLeafExtras组件，用于叶子字段与预览 */}
				<ExitConditionLeafExtras
					condition={leaf}
					requiredBeats={requiredBeats}
					onCommit={commit}
				/>
