...
function EndStoryEntryModeField({
	// value 是当前下一章配置，用于回显 entryMode
	value,
	// onPatch 是激活字段写回，用于同步 entryMode
	onPatch,
}: EndStoryNextActivationPanelProps) {
	return (
...
function EndStoryActivationExtras({
	// value 是当前下一章配置，用于回显提示与提前加锁
	value,
	// onPatch 是激活字段写回，用于同步 hint 与加锁
	onPatch,
}: EndStoryNextActivationPanelProps) {
	return (
		<>
	...
	function EndStoryNextActivationPanel({
		...
		return (
			<div className={styles.panel}>
				...
				/>
				{/* 引用了EndStoryEntryModeField组件，用于入口模式覆盖 */}
				<EndStoryEntryModeField value={value} onPatch={onPatch} />
				{/* 引用了EndStoryActivationExtras组件，用于提示与提前加锁 */}
				<EndStoryActivationExtras value={value} onPatch={onPatch} />
			</div>
		);
	};
