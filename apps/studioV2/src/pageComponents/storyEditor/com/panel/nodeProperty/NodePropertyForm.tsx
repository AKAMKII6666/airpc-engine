...
import { NodePropertyFormFields } from "@studio-v2/src/pageComponents/storyEditor/com/panel/nodeProperty/NodePropertyFormFields";
import { useNodePropertyFormDerived } from "@studio-v2/src/pageComponents/storyEditor/com/panel/nodeProperty/hooks/useNodePropertyFormDerived";
import styles from "./NodePropertyForm.module.scss";

function NodePropertyFormActions({
	// formik 表示浮窗 Formik 实例，用于提交与预览取值
	formik,
	// nodeData 表示当前选中卡投影，用于预览前合并表单
	nodeData,
	// onOpenPreview 表示打开预览，用于写入预览卡并开弹层
	onOpenPreview,
}: {
	formik: FormikProps<NodePropertyFormValues>;
	nodeData: EditorCallCardProjection;
	onOpenPreview: (card: unknown) => void;
}) {
	return (
				...
				onClick={function () {
					onOpenPreview(
						callCardProjectionToDef(
							applyNodePropertyForm(nodeData, formik.values),
						),
					);
				}}
			>
		...
		return (
			<>
				...
				>
					...
					/>
					{/* 引用了NodePropertyFormActions组件，用于提交到画布与打开预览 */}
					<NodePropertyFormActions
						formik={formik}
						nodeData={nodeData}
						onOpenPreview={function (card) {
							setPreviewCard(card);
							setPreviewOpen(true);
						}}
					/>
