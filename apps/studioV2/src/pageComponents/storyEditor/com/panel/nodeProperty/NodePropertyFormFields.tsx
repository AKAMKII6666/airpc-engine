...
import type { NodePropertyFormDerived } from "@studio-v2/src/pageComponents/storyEditor/com/panel/nodeProperty/hooks/useNodePropertyFormDerived";
// 引用了CallCardOwnerSelect组件，用于归属角色下拉
import { CallCardOwnerSelect } from "@studio-v2/src/pageComponents/storyEditor/com/panel/callCard/CallCardOwnerSelect";
// 引用了NodePropertySubModules组件，用于子模块折叠区
import { NodePropertySubModules } from "@studio-v2/src/pageComponents/storyEditor/com/panel/nodeProperty/NodePropertySubModules";
import styles from "./NodePropertyForm.module.scss";

function NodePropertyDetailSections({
	// formik 表示表单实例，用于 AutoForm 与分段标题
	formik,
	// derived 表示派生字段与 items，用于表单编排
	derived,
	// agentId 表示所属角色，用于子模块工具目录
	agentId,
}: {
	...
	return (
		<>
			...
			<Typography variant="caption" className={styles.section}>
				{formik.values.cardKind === "voicemail" ? "目标 · 播放" : "情境 · 目标"}
			</Typography>
	...
	function ({
		...
		return (
			<div className={styles.body}>
				...
				/>
				{/* 引用了NodePropertyDetailSections组件，用于基本信息与子模块 */}
				<NodePropertyDetailSections
					formik={formik}
					derived={derived}
