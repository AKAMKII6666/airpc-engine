...
import { NodePropertyForm } from "@studio-v2/src/pageComponents/storyEditor/com/panel/nodeProperty/NodePropertyForm";
// 引用了ChapterPropertyForm组件，用于章节起止节点属性
import { ChapterPropertyForm } from "@studio-v2/src/pageComponents/storyEditor/com/panel/chapter/ChapterPropertyForm";

function ChapterSelectionForm({
	// selection 是章节选中投影，用于章节表单初值
	selection,
	...
	selection: Extract<StoryEditorSelection, { selectionKind: "chapter" }>;
	onApplyChapterNodeData: FloatingPanelFormBodyProps["onApplyChapterNodeData"];
...
function CallCardSelectionForm({
	// packageId 是路由包键，用于首通提示词预览
	packageId,
	// selection 是通话卡选中投影，用于 CallCard 表单初值
	selection,
	...
	packageId: string;
	selection: Extract<StoryEditorSelection, { selectionKind: "callCard" }>;
	onApplyNodeData: FloatingPanelFormBodyProps["onApplyNodeData"];
	characterAnchors: readonly CharacterAnchorNodeData[];
	effectPanelSources: EffectPanelSources;
	onAssignOwner: FloatingPanelFormBodyProps["onAssignOwner"];
}) {
	return (
				...
				return (
					...
					<NodePropertyForm
						...
						onAssignOwner={function (agentId, displayName) {
							onAssignOwner(selection.nodeId, agentId, displayName);
						}}
					/>
				);
			}}
...
export const FloatingPanelFormBody: FC<FloatingPanelFormBodyProps> =
	function FloatingPanelFormBody({
		...
		if (selection.selectionKind === "chapter") {
			return (
				// 引用了ChapterSelectionForm组件，用于章节属性表单
		...
		return (
			// 引用了CallCardSelectionForm组件，用于 CallCard 属性表单
			<CallCardSelectionForm
				packageId={packageId}
				selection={selection}
