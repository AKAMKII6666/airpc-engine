...
function DockToolButtonList({
	// activeToolId 是当前 toolMode 高亮，用于工具按钮 active 视觉
	activeToolId,
	// onToolClick 转发底栏工具意图，用于壳层归约
	onToolClick,
	// chapterEndDisabled 表示已有章节结束节点，用于禁用该工具
	chapterEndDisabled,
}: {
	activeToolId: DockToolId;
	onToolClick: (toolId: DockToolId) => void;
	...
	return (
		<>
			{DOCK_TOOLS.map((tool) => {
				const disabled = tool.id === "chapter_end" && chapterEndDisabled;
				const active = !disabled && activeToolId === tool.id;
				const tip = disabled ? "本包已有章节结束节点" : tool.label;
				return (
...
export const BottomToolBar: FC<BottomToolBarProps> = function ({
	...
	// onToolClick 转发底栏工具意图，用于壳层归约
	onToolClick,
	// chapterEndDisabled 有章节结束节点时灰掉入口，用于禁用章节结束工具
	chapterEndDisabled,
	// onOpenAssets 打开资源引用浮窗，用于底栏资源入口
	onOpenAssets,
	// onOpenPackage 打开包配置浮窗，用于底栏包配置入口
	onOpenPackage,
	...
	return (
		<footer className={styles.bar} aria-label="画布工具">
			<div className={styles.dock}>
				{/* 引用了DockToolButtonList组件，用于底栏工具模式按钮 */}
				<DockToolButtonList
					activeToolId={activeToolId}
					onToolClick={onToolClick}
					chapterEndDisabled={chapterEndDisabled}
				/>
