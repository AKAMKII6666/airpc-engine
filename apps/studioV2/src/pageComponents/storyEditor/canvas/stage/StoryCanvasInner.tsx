...
import type { StoryCanvasGraphMeta } from "@studio-v2/src/pageComponents/storyEditor/canvas/hooks/useStoryCanvasGraph";
import { useStoryCanvasInnerModel } from "@studio-v2/src/pageComponents/storyEditor/canvas/hooks/useStoryCanvasInnerModel";
import { StoryCanvasUiProvider } from "@studio-v2/src/pageComponents/storyEditor/canvas/stage/storyCanvasUiContext";
...
type StoryCanvasInnerModel = ReturnType<typeof useStoryCanvasInnerModel>;

function StoryCanvasFlowSurface({
	// graph 表示节点边与连线回调，用于 React Flow
	graph,
	// interaction 表示平移框选与光标，用于画布交互
	interaction,
	// onPaneClick 表示空白点击，用于落点或清选中
	onPaneClick,
	// nodeTypes 表示节点渲染表，用于 React Flow
	nodeTypes,
	// edgeTypes 表示边渲染表，用于效果边与结束边
	edgeTypes,
}: {
	graph: StoryCanvasInnerModel["graph"];
	interaction: StoryCanvasInnerModel["interaction"];
	onPaneClick: StoryCanvasInnerModel["onPaneClick"];
	...
	function StoryCanvasInner({
		...
		return (
			...
			<StoryCanvasUiProvider value={uiValue}>
				...
				>
					{/* 引用了StoryCanvasFlowSurface组件，用于故事蓝图画布 */}
					<StoryCanvasFlowSurface
						graph={graph}
