...
function hasPersonaStyleContent(input: {
	profession: string | null;
	speakingStyle: string | null;
	voiceNotes: string | null;
	exampleLines: string[];
}): boolean {
	if (input.profession) return true;
	if (input.speakingStyle) return true;
	if (input.voiceNotes) return true;
	return input.exampleLines.length > 0;
}

function appendPersonaStyleDetailLines(
	lines: string[],
	input: {
		profession: string | null;
		speakingStyle: string | null;
	...
	if (input.profession) lines.push(`- 身份/职业气质：${input.profession}`);
	if (input.speakingStyle) lines.push(`- 说话风格：${input.speakingStyle}`);
	if (input.voiceNotes) lines.push(`- 声音/语气备注：${input.voiceNotes}`);
	if (input.exampleLines.length === 0) return;
	lines.push("- 参考句式（只学节奏和口吻，不要机械复读）：");
	for (const example of input.exampleLines.slice(0, 5)) {
		lines.push(`  - ${example}`);
	}
}
...
export function buildPersonaStyleHardBlock(
	...
	const fields = {
		profession: normalizeText(persona.profession),
		speakingStyle: normalizeText(persona.speakingStyle),
		voiceNotes: normalizeText(persona.voiceNotes),
		exampleLines: normalizeExampleLines(persona.exampleLines),
	};
	if (!hasPersonaStyleContent(fields)) return null;
	const lines = [
		"[persona.style]",
		"角色级说话风格；在 [style.phone] 的电话口语底线内保持角色差异。",
	];
	appendPersonaStyleDetailLines(lines, fields);
	lines.push(
