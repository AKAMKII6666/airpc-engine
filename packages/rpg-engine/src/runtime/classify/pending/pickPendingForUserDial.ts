...
function pickByRedialSlot(
	eligible: CallCardInstance[],
	profile: PlayerProfile,
	agentId: string,
): CallCardInstance | null {
	const slot = profile.telephony?.redialSlot;
	if (!slot || slot.agentId !== agentId || !slot.cardId) return null;
	return (
		eligible.find(function (item) {
			return item.cardId === slot.cardId;
		}) ?? null
	);
}

function comparePendingPriority(a: CallCardInstance, b: CallCardInstance): number {
...
export function pickPendingForIntent(
	...
	const redialHit = pickByRedialSlot(eligible, profile, agentId);
	if (redialHit) return redialHit;
	const sorted = eligible.slice().sort(comparePendingPriority);
