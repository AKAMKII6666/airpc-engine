...
function memoryCommitGateOk(input: {
	session: CallSession;
	outcome: Outcome;
	minTurns: number;
}): boolean {
	const turns = countTurns(input.session);
	if (turns >= input.minTurns) return true;
	if (input.outcome.flags.answered_completed === true) return true;
	return input.session.exitCandidates.length > 0;
}

function skipMemoryCommitReason(input: {
	memory: MemoryPort | null;
	commitEnabled: boolean;
}): FreeCallMemoryCommitResult["skippedReason"] {
	if (!input.memory || !input.commitEnabled) return "memory_disabled";
...
export async function runFreeCallMemoryCommit(input: {
	...
	const gateOk = memoryCommitGateOk({
	...
	if (!gateOk || !commitEnabled || !input.memory) {
		return {
			committed: false,
			skippedReason: skipMemoryCommitReason({
				memory: input.memory,
				commitEnabled,
			}),
		};
