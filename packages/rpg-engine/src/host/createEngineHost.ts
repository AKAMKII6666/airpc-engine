			...
			if (intent.kind === "user_dial" || intent.kind === "agent_outbound") {
				...
				const pinnedInstanceId =
					intent.kind === "agent_outbound" &&
						...
						? intent.instanceId.trim()
						: null;
				const pending = pinnedInstanceId
					? (profile.callCards.board.byAgent[intent.agentId]?.pending.find(
						function (item) {
							return item.instanceId === pinnedInstanceId;
						},
					) ?? null)
					: pickPendingForIntent(profile, intent.agentId, kind, {
