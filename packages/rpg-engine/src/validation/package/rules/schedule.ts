...
function hasRecurringCardRef(effect: {
	scheduleCardId?: unknown;
	cardId?: unknown;
	[key: string]: unknown;
}): { scheduleCardId: string; cardId: string; chapterId: string } | null {
	const scheduleCardId =
		typeof effect.scheduleCardId === "string" ? effect.scheduleCardId : "";
	const cardId = typeof effect.cardId === "string" ? effect.cardId : "";
	const chapterId = resolveChapterId(effect);
	if (!scheduleCardId && !(cardId && chapterId)) return null;
	return { scheduleCardId, cardId, chapterId };
}
...
export async function validateScheduleRecurringEffect(
	...
	const ref = hasRecurringCardRef(effect);
	if (!ref) {
		push(errors, {
	...
	await resolveRecurringTargetKind({
		ref,
		issuePath,
		content,
		workspaceKey,
...
async function resolveRecurringTargetKind(input: {
	ref: { scheduleCardId: string; cardId: string; chapterId: string };
	issuePath: string;
	content: ContentPort;
	workspaceKey: string;
	errors: ValidationIssue[];
}): Promise<void> {
	const { ref, issuePath, content, workspaceKey, errors } = input;
	if (ref.scheduleCardId) {
		await assertScheduleCardViaPort(
			...
			issuePath,
			errors,
			`scheduleCardId ${ref.scheduleCardId}`,
		);
		return;
	}
	if (ref.chapterId === SCHEDULE_CHAPTER_ID) {
		await assertScheduleCardViaPort(
			content,
			workspaceKey,
			ref.cardId,
			issuePath,
			errors,
			`cardId ${ref.cardId} (chapterId=__schedule__)`,
		);
		return;
	}
	if (ref.chapterId === FREE_CHAPTER_ID) {
