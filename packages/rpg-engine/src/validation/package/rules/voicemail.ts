...
function pushCreateVoicemailIfPresent(
	effect: unknown,
	cardPath: string,
	exitId: string,
	errors: ValidationIssue[],
	...
	if (name !== "create_voicemail") return;
...
function scanExitForDeprecatedVoicemail(
	exit: unknown,
	cardPath: string,
	errors: ValidationIssue[],
): void {
	if (typeof exit !== "object" || exit === null) return;
	const exitId = String((exit as { exitId?: unknown }).exitId ?? "?");
	const effects = (exit as { effects?: unknown }).effects;
	if (!Array.isArray(effects)) return;
	for (const effect of effects) {
		pushCreateVoicemailIfPresent(effect, cardPath, exitId, errors);
	}
}
...
export function validateDeprecatedCreateVoicemailInRaw(
	...
	for (const exit of exits) {
		scanExitForDeprecatedVoicemail(exit, cardPath, errors);
	}
}
