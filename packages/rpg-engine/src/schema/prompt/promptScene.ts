...
function layerIdOf(raw: object): string {
	return typeof (raw as { layerId?: unknown }).layerId === "string"
		? (raw as { layerId: string }).layerId
		: "(unknown)";
}

function validateLayerMatchTimeBuckets(
	raw: object,
	layerId: string,
): EngineError | null {
	const match = (raw as { match?: unknown }).match;
	if (typeof match !== "object" || match === null) return null;
	if (!Object.prototype.hasOwnProperty.call(match, "timeBuckets")) return null;
	return engineError(
		"VALIDATION_FAILED",
...
function validateLayerHardPatchKeys(
	raw: object,
	layerId: string,
): EngineError | null {
...
function validateOnePromptSceneLayer(raw: unknown): EngineError | null {
	if (typeof raw !== "object" || raw === null) return null;
	const layerId = layerIdOf(raw);
	return (
		validateLayerMatchTimeBuckets(raw, layerId) ??
		validateLayerHardPatchKeys(raw, layerId)
	);
}
...
export function validatePromptScenePatches(
	...
	for (const raw of layers) {
		const err = validateOnePromptSceneLayer(raw);
		if (err) return err;
	}
	return null;
}
