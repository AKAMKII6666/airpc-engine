...
chapter_start
    ...
    → lanxing_callback_intro「补打自报」
         ...
         exit_voicemail「没说清→留言」→ create_voicemail → lanxing_voicemail
    → lanxing_voicemail「语音留言」→ end_story → chapter_end
...
| 未说清 → 语音信箱 | `create_voicemail` + `lanxing_voicemail` | 留言 effect + 可读卡 |
