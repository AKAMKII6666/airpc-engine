...
| 反例（禁止） | 正例 |
|--------------|------|
| `feature/` 根平铺 Catalog / List / Mock / Confirm 共 ≥4 个异责文件 | `catalog/` `list/` `mock/` `confirm/` 分子目录；根只留薄入口 |
| 已有 `com/`，根上仍堆 20+ 面板 / hooks | 根面板收进职责子目录；根仅 `index` / `types` |
| 实体已进子目录，根上留一排 `export * from "./x"` stub | 删根 stub，调用方改真实路径（禁假搬家） |
| 只加长文件名前缀继续堆在根上 | 按职责进子目录，文件名不必重复整段父路径 |

机检：`STUDIO-STRUCT-008` / `025` / `026`（见 [08 §5.3](./08-Studio-V2前端工程规则与质量门禁需求.md)）。
