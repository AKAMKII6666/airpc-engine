...
**当前版本：`0.4.4`** — 勾选缺口与硬 verify 分轨（`checkbox_missing`）；Executor 绿门禁必须同轮 ✅；BLOCKED 不以 WARN 告警线为主因。
...
命中时若窗口内有「非本仓／不做／禁止／后置／未见／无 Host」等否定语境 → **跳过**（避免「写明不做 Realtime」「未见 Host 写口」反而 BLOCKED）。仍误伤时用更贴行动的 pattern（如 `接入 Host`），或 `--clear-blocked [--after-manual]` 后重跑。
...
默认（非 `--quiet`）时，**本终端会实时刷**:

- `[gbx] Iteration=… Status=…` 阶段切换
- Agent 自身全部 stdout／stderr（含审批提示）
- 仅当 agent **一段时间无输出** 时才出现 `still running (… no output for …s)` 心跳
- `--no-heartbeat` 可关掉心跳行
