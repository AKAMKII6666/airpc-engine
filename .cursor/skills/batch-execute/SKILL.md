...
- 遵循项目 `.cursor/rules/`（分层、测试、文档三层、**注释与复杂 TSX 分区**）
- 每轮 **聚焦** 本轮 ID；禁止「顺便」完成未列入本轮的 ⬜
- 文档更新任务（如 D-*）：若本轮含文档项，只改列出的文档
- 测试：改 Mock/契约逻辑时必须补或改 `tests/`（见任务文件 §8 若有）
- **可读性（本轮触及的复杂编排）：** 新建/改动的 `routes/**/com/**/*.tsx` 或 `page.tsx` 若达复杂阈值，须有 Hooks/States/…/Render 分区；非显然分支旁注原因；导出 props 关键字段 JSDoc。轮末 `npm run check:rules` 若对本轮文件打出 `code-style.client` 分区 warning，视为本轮未完成（须补齐后再勾选）。
- **V1.1 applyOptimization：** 触及该目录时，导出 `interface` 字段级 JSDoc 由 `check:rules` **fail**；缺注释须当场补意图 JSDoc，不得只写文件头/分区过关。
